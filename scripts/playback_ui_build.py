...
# Rename Timeline tab to Playback.
p = Path('src/pages/SessionDetailPage.tsx')
s = p.read_text()
s = s.replace(
...
# Give playback its own volume and follow-playhead controls.
p = Path('src/components/session/timeline/useTimelinePlayback.ts')
s = p.read_text()
s = s.replace("import { useMetronomeStore } from '../../../store/metronome-store';\n", "")
s = s.replace(
...
s = s.replace("  const volUnsubRef = useRef<(() => void) | null>(null);\n", "")
s = s.replace("    volUnsubRef.current?.();\n    volUnsubRef.current = null;\n", "")
s = s.replace(
...
if insert_after not in s:
    raise SystemExit('playback volume insertion target not found')
...
# Replace Timeline UI with a transport-first workbench while preserving existing hooks/editors.
p = Path('src/components/session/TimelineTab.tsx')
original = p.read_text()
imports_end = original.index('interface Props')
imports = original[:imports_end]
tune_start = original.index('function TuneDrawer')
tune_drawer = original[tune_start:]
new_body = r'''interface Props {
...
'''
p.write_text(imports + new_body + tune_drawer)
